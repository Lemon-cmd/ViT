...
class Partitioner(nn.Module):
    def __init__(self, dim, hid_dim, mlp_dim, depth=1, heads=8, dim_head=64, patch_size=16, blk_size=1000, dropout=0.1) -> None:
        ...
        dmodel = hid_dim * (patch_size ** 2)
        self._crossprod = CrossProdMean(dmodel, dim)
        self._pos_embed = nn.Parameter(torch.randn(1, blk_size, dmodel))
        ...
        self._patcher = nn.Sequential(
            ...
            nn.Linear(dim * (patch_size ** 2), dmodel),
            Transformer(dmodel, dim_head, mlp_dim, depth, heads, dropout),
        )
        
        self._reshape = nn.Sequential(
            nn.Linear(dim, dim * (patch_size ** 2)),
